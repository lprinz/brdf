...
//TODO
void TangoData::setMVP(glm::mat4* mvp){
 this->mvp_mat=*(mvp);
}
...
//  pthread_mutex_lock(&TangoData::GetInstance().xyzij_mutex);
  ...
  prev_depth_timestamp = XYZ_ij->timestamp;
   ...
   //TODO
    ...
    //
    TangoData::GetInstance().screenCoord = new float[XYZ_ij->xyz_count*2];
    //TangoData::GetInstance().color_buffer = new float[XYZ_ij->xyz_count*2];

    for (int i=0; i<XYZ_ij->xyz_count;i++){
    glm::vec4 posTrans= glm::vec4(*(&XYZ_ij->xyz[i][0]),*(&XYZ_ij->xyz[i][1]),*(&XYZ_ij->xyz[i][2]),1);
    //transform to camera coordinates
    posTrans = TangoData::GetInstance().mvp_mat * posTrans;
    //normalise
    glm::vec3 pos_ndc = glm::vec3(posTrans.x/posTrans.w,posTrans.y/posTrans.w,posTrans.z/posTrans.w);
    //viewport transformation
    float h = TangoData::GetInstance().height;
    float w= TangoData::GetInstance().width;
    float x = (w/2.0)*pos_ndc.x+(w/2.0);
    float y = (h/2.0)*pos_ndc.y+(h/2.0);
    TangoData::GetInstance().screenCoord[2*i]=x;
    TangoData::GetInstance().screenCoord[2*i+1]=y;
   /* TangoData::GetInstance().color_buffer[3*i]=TangoData::GetInstance().image[3*(int)(x+y*w)];
    TangoData::GetInstance().color_buffer[3*i+1]=TangoData::GetInstance().image[3*(int)(x+y*w)+1];
    TangoData::GetInstance().color_buffer[3*i+2]=TangoData::GetInstance().image[3*(int)(x+y*w)+2];*/
    }
...
//TODO on FRame available
static void connectOnFrameAvailable(void*, TangoCameraId camId,const TangoImageBuffer *buffer) {

 /*pthread_mutex_lock(&TangoData::GetInstance().image_mutex);

   TangoData::GetInstance().height=buffer->height;
  TangoData::GetInstance().width=buffer->width;

   if (TangoData::GetInstance().image == nullptr){
   //TODO: widht height changes? allocation for image
      //TangoData::GetInstance().image = new unsigned char[width*height*3];
   }
...
/*
   std::ofstream myfile;
   myfile.open ("/sdcard/aa.txt");
   myfile << "xxx";
   myfile.close();*/

  // float x=0.0;
  // __android_log_print(ANDROID_LOG_INFO,"out", "%f", x);
...
bool TangoData::ConnectCallbacks() {
  ...
  if (TangoService_connectOnTangoEvent(onTangoEvent) != TANGO_SUCCESS) {
    ...
    //TODO camera image
      if (TangoService_connectOnFrameAvailable (camInt.camera_id, NULL, connectOnFrameAvailable) !=
            TANGO_SUCCESS) {
