...
class TangoData {
 public:
  ...
  float* depth_buffer;
  //TODO
  float* color_buffer= new float[3*1280*720];
  float* screenCoord;
  float* image = new float[3*1280*720];
  uint8_t* image_pointer = new unsigned char[3*1280*720];
    int width =0;
    int height = 0;
  bool is_rgba_dirty;
    TangoCameraIntrinsics camInt;
    TangoImageBuffer camBuffer;
    //
